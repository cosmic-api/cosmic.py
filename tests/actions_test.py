...
class TestBasicAction(TestCase):
    ...
    def setUp(self):
        ...
        self.action = Action(cabbage, returns=ModelNormalizer(JSONData))
...
class TestActionAnnotation(TestCase):
    ...
    def setUp(self):
        self.a_schema = ObjectNormalizer({
            "properties": [
                {
                    "name": "a",
                    "required": True,
                    "schema": IntegerNormalizer()
                },
                {
                    "name": "b",
                    "required": False,
    ...
    def test_args_accepts_incompatible(self):
        ...
        with self.assertRaisesRegexp(SpecError, "incompatible"):
            action = Action(func, accepts=BooleanNormalizer())
